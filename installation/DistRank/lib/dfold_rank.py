...
if __name__=="__main__":
    ...
    parser.add_argument("-r", "--rank", help="generate deeprank features", type=bool, default=False, required=False)
    ...
    for pdb in temp_pdb_file_list:
        if os.path.exists(pdb) == False:
            print("%s file not exist, please check!"%pdb)
            continue
    ...
    for file in files:
        if 'map' in file:
            ...
            tb_dist_name =os.path.basename(tb_dist_file).split('.map')[0]
            ...
            precl2 = precision_score(ab_bin_vec_6, tb_bin_vec_6)
            precl2_long = precision_score(ab_bin_vec_24, tb_bin_vec_24) 
            recall = recall_score(ab_bin_vec_6, tb_bin_vec_6)
            recall_long = recall_score(ab_bin_vec_24, tb_bin_vec_24) 
            #violate
            violate_count = 0
            ...
            recall_long_list.append(recall_long)
    ...
    recall_long_list = np.array(recall_long_list)
    ...
    recall_norm = trans2rank_allow_same(-recall_list)
    recall_long_norm = trans2rank_allow_same(-recall_long_list)
    ...
    for i in range(len(name_list)):
        temp_array = np.array([rmse_norm[i], pearson_norm[i], precl2_norm[i], precl2_long_norm[i], recall_norm[i], recall_long_norm[i], psnr_norm[i], ssim_norm[i], phash_norm[i], gist_norm[i], orb_num_norm[i]])
    ...
    # sum_info_origin = rmse_norm + pearson_norm + precl2_norm + precl2_long_norm + recall_norm + recall_long_norm + psnr_norm + ssim_norm + phash_norm + gist_norm + orb_num_norm
    sum_info_origin = rmse_norm + pearson_norm + precl2_norm + recall_norm + psnr_norm + ssim_norm + phash_norm + gist_norm + orb_num_norm
    for i in range(len(name_list)):
        with open(info_txt, 'a') as myfile:
            str_to_write = "%s\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n"%(name_list[i], rmse_list[i], pearson_list[i], 
                precl2_list[i], precl2_long_list[i], recall_list[i], recall_long_list[i], psnr_list[i], ssim_list[i], phash_list[i], gist_list[i], orb_num_list[i], sum_info_origin[i], sum_info[i])
            myfile.write(str_to_write)
    if if_gen_fea4rank == True or if_gen_fea4rank == 'True':
        rmse_fea = outdir +'/feature_dist_rmse.%s'%seq_name
        pearson_fea = outdir +'/feature_dist_pearson.%s'%seq_name
        precl2_fea = outdir +'/feature_dist_precl2.%s'%seq_name
        precl2_long_fea = outdir +'/feature_dist_precl2_long.%s'%seq_name
        recall_fea = outdir +'/feature_dist_recall.%s'%seq_name
        recall_long_fea = outdir +'/feature_dist_recall_long.%s'%seq_name
        psnr_fea = outdir +'/feature_dist_psnr.%s'%seq_name
        ssim_fea = outdir +'/feature_dist_ssim.%s'%seq_name
        phash_fea = outdir +'/feature_dist_phash.%s'%seq_name
        gist_fea = outdir +'/feature_dist_gist.%s'%seq_name
        orb_num_fea = outdir +'/feature_dist_orb_num.%s'%seq_name
        if os.path.exists(rmse_fea):os.remove(rmse_fea)
        if os.path.exists(pearson_fea):os.remove(pearson_fea)
        if os.path.exists(precl2_fea):os.remove(precl2_fea)
        if os.path.exists(precl2_long_fea):os.remove(precl2_long_fea)
        if os.path.exists(recall_fea):os.remove(recall_fea)
        if os.path.exists(recall_long_fea):os.remove(recall_long_fea)
        if os.path.exists(psnr_fea):os.remove(psnr_fea)
        if os.path.exists(ssim_fea):os.remove(ssim_fea)
        if os.path.exists(phash_fea):os.remove(phash_fea)
        if os.path.exists(gist_fea):os.remove(gist_fea)
        if os.path.exists(orb_num_fea):os.remove(orb_num_fea)
        for i in range(len(name_list)):
            # print(name_list[i])
            with open(rmse_fea, 'a') as myfile:
                str_to_write = "%s\t%.4f\t\n"%(name_list[i], rmse_list[i])
                myfile.write(str_to_write)
            with open(pearson_fea, 'a') as myfile:
                str_to_write = "%s\t%.4f\t\n"%(name_list[i], pearson_list[i])
                myfile.write(str_to_write)
            with open(precl2_fea, 'a') as myfile:
                str_to_write = "%s\t%.4f\t\n"%(name_list[i], precl2_list[i])
                myfile.write(str_to_write)
            with open(precl2_long_fea, 'a') as myfile:
                str_to_write = "%s\t%.4f\t\n"%(name_list[i], precl2_long_list[i])
                myfile.write(str_to_write)
            with open(recall_fea, 'a') as myfile:
                str_to_write = "%s\t%.4f\t\n"%(name_list[i], recall_list[i])
                myfile.write(str_to_write)
            with open(recall_long_fea, 'a') as myfile:
                str_to_write = "%s\t%.4f\t\n"%(name_list[i], recall_long_list[i])
                myfile.write(str_to_write)
            with open(psnr_fea, 'a') as myfile:
                str_to_write = "%s\t%.4f\t\n"%(name_list[i], psnr_list[i])
                myfile.write(str_to_write)
            with open(ssim_fea, 'a') as myfile:
                str_to_write = "%s\t%.4f\t\n"%(name_list[i], ssim_list[i])
                myfile.write(str_to_write)
            with open(phash_fea, 'a') as myfile:
                str_to_write = "%s\t%.4f\t\n"%(name_list[i], phash_list[i])
                myfile.write(str_to_write)
            with open(gist_fea, 'a') as myfile:
                str_to_write = "%s\t%.4f\t\n"%(name_list[i], gist_list[i])
                myfile.write(str_to_write)
            with open(orb_num_fea, 'a') as myfile:
                str_to_write = "%s\t%.4f\t\n"%(name_list[i], orb_num_list[i])
                myfile.write(str_to_write)
    ...
    for i in range(len(rank_index)):
        ...
        with open(rank_txt, 'a') as myfile:
            str_to_write = "%s\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n"%(name_list[rank_index[i]], sum_info_origin[rank_index[i]], precl2_list[rank_index[i]], precl2_long_list[rank_index[i]], recall_list[rank_index[i]], recall_long_list[rank_index[i]])
            myfile.write(str_to_write)
