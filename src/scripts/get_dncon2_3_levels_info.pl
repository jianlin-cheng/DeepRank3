...
foreach my $rr (@rr_list){
  if ($seq_rr{$rr} ne $seq_pdb){
    ...
    die "Please check your model and sequence\n\n";
  }
}
...
#print Dumper(\%all_rr);
#open TEMP, ">$output_folder/${seq_id}_$range.txt" or confess $!;
#while (my ($k,$v)=each $all_rr{"".$base_rr." "."L/5"}){
#  print TEMP "$v\n";
#}
#close TEMP;
...
foreach my $rr (sort @rr_list){
  ...
  foreach my $top (sort {$top_count{$a} <=> $top_count{$b}} keys %top_count){
    ...
    if($top eq "L/5"){
      my $cal_pre=${in_pdb_file}.":".$range.":".calc_precision(\%this_rr);
      if(index(basename($rr),'.rr')>=0) ## added by Jie
      {
        print TEMP "$cal_pre\n";
      }
    }
...
sub seq_rr{
  ...
  while(<RR>){
    ...
    next if ($_ =~ /^MODEL/);
    next if ($_ =~ /^PARENT/);
    last if ($_ =~ /^TER/ and length($_) == 3);
    last if ($_ =~ /^END/ and length($_) == 3);
