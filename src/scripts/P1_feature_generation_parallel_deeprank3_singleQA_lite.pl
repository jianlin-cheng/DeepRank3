...
my $models_dist = $TMP_output."/mod_dist";
if(-d $models_dist)
{
        `rm -rf  $models_dist`;
}
`cp -ar $dir_models $models_dist`;

open(MLIST, ">$TMP_output/$targetname.mlist");
opendir(MDIR, $models_dist);
...
`ln -s $dir_models $models_proq3`;


my($models_local_global) = $TMP_output."/mod_locglo";
if(-l $models_local_global)
{
	`rm $models_local_global`;
}
`ln -s $dir_models  $models_local_global`;
...
# 	`rm  $models_dncon2`;
# }
# `ln -s $dir_models $models_dncon2`;

my($models_dncon4) = $TMP_output."/mod_dncon4";
if(-l $models_dncon4)
{
        `rm  $models_dncon4`;
}
`ln -s $dir_models $models_dncon4`;
...
my($ren_dncon4_features)=$TMP_output."/dncon4";
-d $ren_dncon4_features || `mkdir $ren_dncon4_features`;
$ren_dncon4_short=$TMP_output."/"."feature_dncon4_short-range.".$targetname;
$ren_dncon4_medium=$TMP_output."/"."feature_dncon4_medium-range.".$targetname;
$ren_dncon4_long=$TMP_output."/"."feature_dncon4_long-range.".$targetname;


$ren_dist_gist=$TMP_output."/"."feature_dist_gist.".$targetname;
$ren_dist_precl2_long=$TMP_output."/"."feature_dist_precl2_long.".$targetname;
$ren_dist_precl2=$TMP_output."/"."feature_dist_precl2.".$targetname;
$ren_dist_psnr=$TMP_output."/"."feature_dist_psnr.".$targetname;
$ren_dist_recall_long=$TMP_output."/"."feature_dist_recall_long.".$targetname;
$ren_dist_gist=$TMP_output."/"."feature_dist_gist.".$targetname;
$ren_dist_orb_num=$TMP_output."/"."feature_dist_orb_num.".$targetname;
$ren_dist_pearson=$TMP_output."/"."feature_dist_pearson.".$targetname;
$ren_dist_phash=$TMP_output."/"."feature_dist_phash.".$targetname;
...
$tools2		="OPUS,RF_SRS,RWplus,dope,voronota,SBROD,deepdist";
@tools		=split(/,/,$tools2);

$post_process = 0; 
$thread_num = @tools;
%thread_ids = ();
$EXEC_LIMIT_HRS = 60*60*24*2;#24 hrs
...
for($i = 0; $i < @tools; $i++)
{
	...
	}elsif($kidpid == 0)
	{
		...
		}elsif ($tool eq "SBROD") ### 17. generating Proq3D score
		{
			...
			close TMP;		
			exit 0;
		}elsif ($tool eq "dncon4") ### 17. generating dncon4 score
                {
                        $dncon4_starttime = time();
                        my($ren_dncon4_features)=$TMP_output."/dncon4";
                        if(!(-d $ren_dncon4_features))
                        {
                                `mkdir $ren_dncon4_features`;
                        }
                        `cp $dncon4file  $ren_dncon4_features/$targetname.dncon4.rr`;


                        if(-e "$dir_output/ALL_scores/feature_dncon4_short-range.$targetname" and -e "$dir_output/ALL_scores/feature_dncon4_medium-range.$targetname" and -e "$dir_output/ALL_scores/feature_dncon4_long-range.$targetname")
                        {
                                print "19. $dir_output/ALL_scores/feature_dncon4_short-range.$targetname already generated! Pass\n";
                                print "19. $dir_output/ALL_scores/feature_dncon4_medium-range.$targetname already generated! Pass\n";
                                print "19. $dir_output/ALL_scores/feature_dncon4_long-range.$targetname already generated! Pass\n";
                                `cp $dir_output/ALL_scores/feature_dncon4_short-range.$targetname  $ren_dncon4_short`;
                                `cp $dir_output/ALL_scores/feature_dncon4_medium-range.$targetname  $ren_dncon4_medium`;
                                `cp $dir_output/ALL_scores/feature_dncon4_long-range.$targetname  $ren_dncon4_long`;
                        }else{
                          		$res = "$LOG_output/dncon4.is_running";
                                
                                print "####### 1. Predicting contact\n\n";

                                my($score_dncon4_short) = $ren_dncon4_features."/${targetname}_contact_energy/".$targetname.'.short-range';
                                my($score_dncon4_medium) = $ren_dncon4_features."/${targetname}_contact_energy/".$targetname.'.medium-range';
                                my($score_dncon4_long) = $ren_dncon4_features."/${targetname}_contact_energy/".$targetname.'.long-range';
                                
                                if(-e $score_dncon4_short and -e $score_dncon4_medium and -e $score_dncon4_long)
                                {
                                        print "$score_dncon4_short generated!\n\n";
                                        print "$score_dncon4_medium generated!\n\n";
                                        print "$score_dncon4_long generated!\n\n";
                                }else{
                                        $cmd = "perl $H_script/CASP13_run_contact_energy.pl $ren_dncon4_features/$targetname.dncon4.rr  $models_dncon4  $targetname $ren_dncon4_features";
                                        $OUT = new FileHandle ">$res";
                                        print $OUT "19. generating contact energy score\n   $cmd \n\n";
                                        print  "19. generating contact energy score\n   $cmd \n\n";
                                        $OUT->close();
                                        ...
                                        if ($ren_return_val)
                                        {
                                                        $dncon4_finishtime = time();
                                                        $dncon4_diff_hrs = ($dncon4_finishtime - $dncon4_starttime)/3600;
                                                system("mv $LOG_output/dncon4.is_running $LOG_output/dncon4.is_finished");
                                                open(TMP,">>$LOG_output/dncon4.is_finished");
                                                print TMP "ERROR! dncon4 execution <$cmd> failed!\n";
                                                print TMP "dncon4 modeling finished within $dncon4_diff_hrs hrs!\n\n";
                                                close TMP;                              
                                                print "ERROR! dncon4 execution failed!";
                                                exit 0;
                                        }
                                }
                                
                                
                           if(-e $score_dncon4_short)
                           {
                                  `cp $score_dncon4_short $ren_dncon4_short`;
                                  system("cp $ren_dncon4_short $ALL_scores");
                                  system("cp $ren_dncon4_short $ALL_14_scores");
                           }
                           
                                
                           if(-e $score_dncon4_medium)
                           {
                                  `cp $score_dncon4_medium $ren_dncon4_medium`;
                                  system("cp $ren_dncon4_medium $ALL_scores");
                                  system("cp $ren_dncon4_medium $ALL_14_scores");
                           }
                           
                                
                           if(-e $score_dncon4_long)
                           {
                                  `cp $score_dncon4_long $ren_dncon4_long`;
                                  system("cp $ren_dncon4_long $ALL_scores");
                                  system("cp $ren_dncon4_long $ALL_14_scores");
                           }
                           
                           if(-e $ren_dncon4_long and -e $ren_dncon4_medium and -e $ren_dncon4_short)
                           {
                                        print "!!Successfully generated $ren_dncon4_short, $ren_dncon4_medium, $ren_dncon4_short\n\n";
                        ...
                        $dncon4_finishtime = time();
                        $dncon4_diff_hrs = ($dncon4_finishtime - $dncon4_starttime)/3600;
                        print "19. dncon4 modeling finished within $dncon4_diff_hrs hrs!\n\n";
                        if(-e "$LOG_output/dncon4.is_running")
                        {
                                system("mv $LOG_output/dncon4.is_running $LOG_output/dncon4.is_finished");
                        }
                        ...
                        close TMP;              
                        exit 0;
       	}elsif($tool eq "deepdist") ### 17. generating deepdist score
        {
						$deepdist_starttime = time();
						if(-e "$ren_dncon4_features/$targetname.dncon4.rr" and -e "$TMP_output/$targetname.distmap")
						{
							print "$ren_dncon4_features/$targetname.dncon4.rr and $TMP_output/$targetname.distmap generated!\n\n";
						}
						else
						{
							...
							if ($ren_return_val)
							{
								$deepdist_finishtime = time();
								$deepdist_diff_hrs = ($deepdist_finishtime - $deepdist_starttime)/3600;
								print "19. deppdist finished within $deepdist_diff_hrs hrs!\n\n";

								system("mv $LOG_output/deepdist.is_running $LOG_output/deepdist.is_finished");
								open(TMP,">>$LOG_output/dncon4.is_finished");
								print TMP "ERROR! deepdist execution <$cmd> failed!\n";
								print TMP "dncon4 modeling finished within $deepdist_diff_hrs hrs!\n\n";
								close TMP;
								print "ERROR! deepdist execution failed!";
								exit 0;
							}
							$dncon4file = "$ren_deepdist_features/pred_map_ensem/rr/$targetname.rr";
							$distfile = "$ren_deepdist_features/pred_map_ensem/$targetname.txt";
							`cp $dncon4file  $ren_dncon4_features/$targetname.dncon4.rr`;
							`cp $distfile $TMP_output/$targetname.distmap`;
						}

                        $distrank_starttime = time();
                        my($ren_distrank_features)=$TMP_output."/distrank";
                        if(!(-d $ren_distrank_features))
                        {
                                `mkdir $ren_distrank_features`;
                        }


						if(-e "$dir_output/ALL_scores/feature_dist_gist.$targetname" and -e "$dir_output/ALL_scores/feature_dist_precl2_long.$targetname" and -e "$dir_output/ALL_scores/feature_dist_precl2.$targetname" and -e "$dir_output/ALL_scores/feature_dist_psnr.$targetname" and -e "$dir_output/ALL_scores/feature_dist_recall_long.$targetname"  and -e "$dir_output/ALL_scores/feature_dist_orb_num.$targetname" and -e "$dir_output/ALL_scores/feature_dist_pearson.$targetname" and -e "$dir_output/ALL_scores/feature_dist_phash.$targetname" and -e "$dir_output/ALL_scores/feature_dist_recall.$targetname" and -e "$dir_output/ALL_scores/feature_dist_rmse.$targetname" and -e "$dir_output/ALL_scores/feature_dist_ssim.$targetname")
						{
							print "20. $dir_output/ALL_scores/feature_dist_gist.$targetname already generated! Pass\n";
							print "20. $dir_output/ALL_scores/feature_dist_precl2_long.$targetname already generated! Pass\n";
							print "20. $dir_output/ALL_scores/feature_dist_precl2.$targetname already generated! Pass\n";
							print "20. $dir_output/ALL_scores/feature_dist_psnr.$targetname already generated! Pass\n";
							print "20. $dir_output/ALL_scores/feature_dist_recall_long.$targetname already generated! Pass\n";
							print "20. $dir_output/ALL_scores/feature_dist_orb_num.$targetname already generated! Pass\n";
							print "20. $dir_output/ALL_scores/feature_dist_pearson.$targetname already generated! Pass\n";
							print "20. $dir_output/ALL_scores/feature_dist_phash.$targetname already generated! Pass\n";
							print "20. $dir_output/ALL_scores/feature_dist_recall.$targetname already generated! Pass\n";
							print "20. $dir_output/ALL_scores/feature_dist_rmse.$targetname already generated! Pass\n";
							print "20. $dir_output/ALL_scores/feature_dist_ssim.$targetname already generated! Pass\n";
						}
						else
						{
                          	$res = "$LOG_output/distrank.is_running";

                            print "####### 1. Predicting distance feature\n\n";

							$cmd = "sh $H_tool/DistRank/scripts/run_distrank.sh $fasta_seq  $TMP_output/$targetname.mlist  $TMP_output/$targetname.distmap  $ren_distrank_features";
							$OUT = new FileHandle ">$res";
							print $OUT "19. generating distance score\n   $cmd \n\n";
							print  "19. generating distance score\n   $cmd \n\n";
							$OUT->close();
							$ren_return_val=system("$cmd &>> $res");
							if ($ren_return_val)
							{
								$distrank_finishtime = time();
								$distrank_diff_hrs = ($distrank_finishtime - $distrank_starttime)/3600;
								system("mv $LOG_output/distrank.is_running $LOG_output/distrank.is_finished");
								open(TMP,">>$LOG_output/distrank.is_finished");
								print TMP "ERROR! distrank execution <$cmd> failed!\n";
								print TMP "distrank finished within $distrank_diff_hrs hrs!\n\n";
								close TMP;
								print "ERROR! distrank execution failed!";
								exit 0;
							}


							$score_dist_gist=$ren_distrank_features."/"."feature_dist_gist.".$targetname;
							$score_dist_precl2_long=$ren_distrank_features."/"."feature_dist_precl2_long.".$targetname;
							$score_dist_precl2=$ren_distrank_features."/"."feature_dist_precl2.".$targetname;
							$score_dist_psnr=$ren_distrank_features."/"."feature_dist_psnr.".$targetname;
							$score_dist_recall_long=$ren_distrank_features."/"."feature_dist_recall_long.".$targetname;
							$score_dist_orb_num=$ren_distrank_features."/"."feature_dist_orb_num.".$targetname;
							$score_dist_pearson=$ren_distrank_features."/"."feature_dist_pearson.".$targetname;
							$score_dist_phash=$ren_distrank_features."/"."feature_dist_phash.".$targetname;
							$score_dist_recall=$ren_distrank_features."/"."feature_dist_recall.".$targetname;
							$score_dist_rmse=$ren_distrank_features."/"."feature_dist_rmse.".$targetname;
							$score_dist_ssim=$ren_distrank_features."/"."feature_dist_ssim.".$targetname;

							if(-e $score_dist_gist)
							{
								`cp $score_dist_gist $ren_dist_gist`;
								system("cp $ren_dist_gist $ALL_scores");
								system("cp $ren_dist_gist $ALL_14_scores");
							}

							if(-e $score_dist_precl2_long)
							{
								`cp $score_dist_precl2_long $ren_dist_precl2_long`;
								system("cp $ren_dist_precl2_long $ALL_scores");
								system("cp $ren_dist_precl2_long $ALL_14_scores");
							}

							if(-e $score_dist_precl2)
							{
								`cp $score_dist_precl2 $ren_dist_precl2`;
								system("cp $ren_dist_precl2 $ALL_scores");
								system("cp $ren_dist_precl2 $ALL_14_scores");
							}

							if(-e $score_dist_psnr)
							{
								`cp $score_dist_psnr $ren_dist_psnr`;
								system("cp $ren_dist_psnr $ALL_scores");
								system("cp $ren_dist_psnr $ALL_14_scores");
							}

							if(-e $score_dist_recall_long)
							{
								`cp $score_dist_recall_long $ren_dist_recall_long`;
								system("cp $ren_dist_recall_long $ALL_scores");
								system("cp $ren_dist_recall_long $ALL_14_scores");
							}

							if(-e $score_dist_orb_num)
							{
								`cp $score_dist_orb_num $ren_dist_orb_num`;
								system("cp $ren_dist_orb_num $ALL_scores");
								system("cp $ren_dist_orb_num $ALL_14_scores");
							}

							if(-e $score_dist_pearson)
							{
								`cp $score_dist_pearson $ren_dist_pearson`;
								system("cp $ren_dist_pearson $ALL_scores");
								system("cp $ren_dist_pearson $ALL_14_scores");
							}

							if(-e $score_dist_phash)
							{
								`cp $score_dist_phash $ren_dist_phash`;
								system("cp $ren_dist_phash $ALL_scores");
								system("cp $ren_dist_phash $ALL_14_scores");
							}

							if(-e $score_dist_recall)
							{
								`cp $score_dist_recall $ren_dist_recall`;
								system("cp $ren_dist_recall $ALL_scores");
								system("cp $ren_dist_recall $ALL_14_scores");
							}

							if(-e $score_dist_rmse)
							{
								`cp $score_dist_rmse $ren_dist_rmse`;
								system("cp $ren_dist_rmse $ALL_scores");
								system("cp $ren_dist_rmse $ALL_14_scores");
							}

							if(-e $score_dist_ssim)
							{
								`cp $score_dist_ssim $ren_dist_ssim`;
								system("cp $ren_dist_ssim $ALL_scores");
								system("cp $ren_dist_ssim $ALL_14_scores");
							}

							if(-e "$dir_output/ALL_scores/feature_dist_gist.$targetname" and -e "$dir_output/ALL_scores/feature_dist_precl2_long.$targetname" and -e "$dir_output/ALL_scores/feature_dist_precl2.$targetname" and -e "$dir_output/ALL_scores/feature_dist_psnr.$targetname" and -e "$dir_output/ALL_scores/feature_dist_recall_long.$targetname"  and -e "$dir_output/ALL_scores/feature_dist_orb_num.$targetname" and -e "$dir_output/ALL_scores/feature_dist_pearson.$targetname" and -e "$dir_output/ALL_scores/feature_dist_phash.$targetname" and -e "$dir_output/ALL_scores/feature_dist_recall.$targetname" and -e "$dir_output/ALL_scores/feature_dist_rmse.$targetname" and -e "$dir_output/ALL_scores/feature_dist_ssim.$targetname")
							{
								print "!!Successfully generated distance feature scores\n\n";
							}
							else
							{
								print "!!Failed to generate distance feature scores\n\n";
							}
                        }

                        $distrank_finishtime = time();
                        $distrank_diff_hrs = ($distrank_finishtime - $distrank_starttime)/3600;
                        print "20. distrank finished within $distrank_diff_hrs hrs!\n\n";
                        if(-e "$LOG_output/distrank.is_running")
                        {
                                system("mv $LOG_output/distrank.is_running $LOG_output/distrank.is_finished");
                        }
...
$dncon4_manuallystop = 0;
$distmap_manuallystop = 0;
...
if ($i == $thread_num && $post_process == 0)
{
	...
	{	
		...
		if($dncon4_finish)
                {
                        ...
                        $checkfile1 = "$dir_output/ALL_scores/feature_dncon4_short-range.$targetname";
                        $checkfile2 = "$dir_output/ALL_scores/feature_dncon4_medium-range.$targetname";
                        $checkfile3= "$dir_output/ALL_scores/feature_dncon4_long-range.$targetname";
                        if((-e $checkfile1 and -e $checkfile2 and -e $checkfile3) or $dncon4_manuallystop == 1)
                        {
                                if(-e $checkfile1 and -e $checkfile2 and -e $checkfile3)
                                {
                                        $dncon4_finish = 0;
                                ...
                                if($dncon4_manuallystop == 0)
                                {
                                        print "dncon4 modeling finished!\n";
                                        $finishtime = time();
                                        $model_diff_hrs = ($finishtime - $checktime)/3600;
                                        open(STATUS,">>$dir_output/modelling_status.txt")|| die "Failed to write $dir_output/modelling_status.txt \n";
                                        print STATUS "dncon4 modeling finished! ($model_diff_hrs hr)\n";
                                        close STATUS;
                                        $dncon4_finish = 0;
                                        next;
                                }else{
                                        print "dncon4 modeling took >$EXEC_LIMIT_HRS hr! Automatically stopped!\n";
                                        $finishtime = time();
                                        $model_diff_hrs = ($finishtime - $checktime)/3600;
                                        open(STATUS,">>$dir_output/modelling_status.txt")|| die "Failed to write $dir_output/modelling_status.txt \n";
                                        print STATUS "dncon4 modeling took >$EXEC_LIMIT_HRS hr! Automatically stopped! Ran ($model_diff_hrs hr)\n";
                                        close STATUS;   
                                        $dncon4_finish = 0;
                                        next;                   
                                }
	 	...
	 	if($distmap_finish)
                {
			$checkfile1="$dir_output/ALL_scores/feature_dist_gist.".$targetname;
			$checkfile2="$dir_output/ALL_scores/feature_dist_precl2_long.".$targetname;
			$checkfile3="$dir_output/ALL_scores/feature_dist_precl2.".$targetname;
			$checkfile4="$dir_output/ALL_scores/feature_dist_psnr.".$targetname;
			$checkfile5="$dir_output/ALL_scores/feature_dist_recall_long.".$targetname;
			$checkfile6="$dir_output/ALL_scores/feature_dist_orb_num.".$targetname;
			$checkfile7="$dir_output/ALL_scores/feature_dist_pearson.".$targetname;
			$checkfile8="$dir_output/ALL_scores/feature_dist_phash.".$targetname;
			$checkfile9="$dir_output/ALL_scores/feature_dist_recall.".$targetname;
			$checkfile10="$dir_output/ALL_scores/feature_dist_rmse.".$targetname;
			$checkfile11="$dir_output/ALL_scores/feature_dist_ssim.".$targetname;

                        if((-e $checkfile1 and -e $checkfile2 and -e $checkfile3 and -e $checkfile4 and -e $checkfile5 and -e $checkfile6 and -e $checkfile7 and -e $checkfile8 and -e $checkfile9 and -e $checkfile10 and -e $checkfile11) or $distmap_manuallystop == 1)
                        {
                                if(-e $checkfile1 and -e $checkfile2 and -e $checkfile3 and -e $checkfile4 and -e $checkfile5 and -e $checkfile6 and -e $checkfile7 and -e $checkfile8 and -e $checkfile9 and -e $checkfile10 and -e $checkfile11)
                                {
                                        $distmap_finish = 0;
                                        next;
                                }
                                if($distmap_manuallystop == 0)
                                {
                                        print "distmap modeling finished!\n";
                                        $finishtime = time();
                                        $model_diff_hrs = ($finishtime - $checktime)/3600;
                                        open(STATUS,">>$dir_output/modelling_status.txt")|| die "Failed to write $dir_output/modelling_status.txt \n";
                                        print STATUS "distmap modeling finished! ($model_diff_hrs hr)\n";
                                        close STATUS;
                                        $distmap_finish = 0;
                                        next;
                                }else{
                                        print "distmap modeling took >$EXEC_LIMIT_HRS hr! Automatically stopped!\n";
                                        $finishtime = time();
                                        $model_diff_hrs = ($finishtime - $checktime)/3600;
                                        open(STATUS,">>$dir_output/modelling_status.txt")|| die "Failed to write $dir_output/modelling_status.txt \n";
                                        print STATUS "distmap modeling took >$EXEC_LIMIT_HRS hr! Automatically stopped! Ran ($model_diff_hrs hr)\n";
                                        close STATUS;   
                                        $dncon4_finish = 0;
                                        next;                   
                                }
		...
		print "!!!!!! OPUS:".$OPUS_finish."\tlocal_global:".$local_global_finish."\tRF_SRS:".$RF_SRS_finish."\tRWplus:".$RWplus_finish."\tdope:".$dope_finish."\tvoronota:".$voronota_finish."\tDeepQA:".$DeepQA_finish."\tProq3:".$Proq3_finish."\tSBROD:".$SBROD_finish."\tdncon2:".$dncon2_finish."\tdncon4:".     $dncon4_finish."\tdistmap:".$distmap_finish." !!!!!! \n\n";
			
		if($OPUS_finish== 0 and $local_global_finish== 0 and $RF_SRS_finish== 0 and $RWplus_finish== 0 and $dope_finish== 0 and $voronota_finish== 0 and $DeepQA_finish== 0 and $Proq3_finish== 0 and $SBROD_finish== 0 and $dncon2_finish== 0 and $dncon4_finish==0 and $distmap_finish==0)
		{
			last;
		}
		...
		#if($model_check_hrs > 60*3)
		{
			for ($k = 0; $k < @tools; $k++)
			{
				...
				if($tool eq 'deepdist')
                {
                    $distmap_manuallystop = 1;
					$dncon4_manuallystop = 1;
                }
			}
		}	
		sleep(600);
...
# system("cp $ren_dncon2_long $ALL_14_scores");
 ...
 system("rm  $models_proq3");
 system("rm  $models_folder");
 system("rm  $models_DeepQA");
 system("rm  $models_dncon2");
 system("rm  $models_local_global");
