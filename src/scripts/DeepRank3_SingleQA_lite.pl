...
print "\n\n##### (1) Run feature generation\n\n";
print("perl $H_script/P1_feature_generation_parallel_deeprank3_singleQA_lite.pl $targetname $seqfile $dir_models $dir_output $dncon4_file $dist_file\n");
$status = system("perl $H_script/P1_feature_generation_parallel_deeprank3_singleQA_lite.pl $targetname $seqfile $dir_models $dir_output $dncon4_file $dist_file");
...
#{
	...
	print("perl $H_script/P2_combine_score_into_SVMformat_nosubfix_fea17.pl $dir_models $dir_output/ALL_scores/ $targetname  $dir_output/feat_singleQA_lite.txt $native_score\n");
	$status = system("perl $H_script/P2_combine_score_into_SVMformat_nosubfix_fea17.pl $dir_models $dir_output/ALL_scores/ $targetname  $dir_output/feat_singleQA_lite.txt $native_score");
	if($status)# if failed, should we use at least one score?
	{
	...
	print "\n\n##### (3) Run make prediction for gdt score\n\n";
	`mkdir $dir_output/10_fold_prediction_singleQA_lite/`;
	print("perl $H_script/P3_make_prediction_by_10folds.pl $dir_output/feat_singleQA_lite.txt test $H_script/python_lib/DeepRank3_singleQA_fea17_models/ $dir_output/10_fold_prediction_singleQA_lite/\n");
	$status = system("perl $H_script/P3_make_prediction_by_10folds.pl $dir_output/feat_singleQA_lite.txt test $H_script/python_lib/DeepRank3_singleQA_fea17_models/ $dir_output/10_fold_prediction_singleQA_lite/");
	if($status)# if failed, should we use at least one score?
	{
	...
	print("perl $H_script/P4_add_prediction2feature_feature18_10folds_avg.pl $dir_output/feat_singleQA_lite.txt  test $dir_output/10_fold_prediction_singleQA_lite/  $dir_output/feat_with10folds_singleQA_lite.txt\n");
	$status = system("perl $H_script/P4_add_prediction2feature_feature18_10folds_avg.pl $dir_output/feat_singleQA_lite.txt  test $dir_output/10_fold_prediction_singleQA_lite/  $dir_output/feat_with10folds_singleQA_lite.txt");
	if($status)# if failed, should we use at least one score?
	{
		die "Failed to run <P1_add_prediction2feature_feature31_10folds.pl>\n";
	}

	print("THEANO_FLAGS=floatX=float32,device=cpu python $H_script/python_lib/predict_main_auto_NN_single.py nadam $dir_output/feat_with10folds_singleQA_lite.txt $H_script/python_lib/DeepRank3_singleQA_fea17_models/10fold_2nd_stage_avg_optnadam_htsigmoid_hn10   $dir_output/feat_gdt_predict_singleQA_lite.txt\n");
	$status = system("THEANO_FLAGS=floatX=float32,device=cpu python $H_script/python_lib/predict_main_auto_NN_single.py nadam $dir_output/feat_with10folds_singleQA_lite.txt $H_script/python_lib/DeepRank3_singleQA_fea17_models/10fold_2nd_stage_avg_optnadam_htsigmoid_hn10   $dir_output/feat_gdt_predict_singleQA_lite.txt");
	if($status)# if failed, should we use at least one score?
	{
		die "Failed to run the prediction for gdt\n";
	}
	...
	print("perl $H_script/P5_add_prediction2feature_feature18.pl $dir_output/feat_singleQA_lite.txt   $dir_output/feat_gdt_predict_singleQA_lite.txt  $dir_output/feat_gdt_predict_withall_SingleQA_lite.txt\n");
	$status = system("perl $H_script/P5_add_prediction2feature_feature18.pl $dir_output/feat_singleQA_lite.txt   $dir_output/feat_gdt_predict_singleQA_lite.txt  $dir_output/feat_gdt_predict_withall_SingleQA_lite.txt");
	if($status)# if failed, should we use at least one score?
	{
		die "Failed to run the prediction adding for gdt\n";
	}

	$status = system("perl $H_script/P6_get_model2prediction.pl $dir_output/feat_singleQA_lite.txt  $dir_output/feat_gdt_predict_singleQA_lite.txt  $dir_output/DeepRank3_SingleQA_lite_unsort.txt");
	if($status)# if failed, should we use at least one score?
	{
		die "Failed to get final prediction for gdt\n";
	}
	
	system("perl $H_script/sort_deep_qa_score.pl $dir_output/DeepRank3_SingleQA_lite_unsort.txt $dir_output/DeepRank3_SingleQA_lite.txt");
	...
	if($native_score ne 'None')
	{
		...
		system("perl $H_script/P7_evaluate_feature_by_targets.pl  $dir_output/feat_gdt_predict_withall_SingleQA_lite.txt  $dir_output/feature_analysis/  $dir_output/feat_gdt_predict_withall_SingleQA_lite.eva");
	}
...
print "\nFinal prediction: $dir_output/DeepRank3_SingleQA_lite.txt\n\n";
