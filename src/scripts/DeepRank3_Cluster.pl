...
print "\n\n##### (1) Run feature generation\n\n";
print("perl $H_script/P1_feature_generation_parallel_deeprank3.pl $targetname $seqfile $dir_models $dir_output $dncon4_file $dist_file\n");
$status = system("perl $H_script/P1_feature_generation_parallel_deeprank3.pl $targetname $seqfile $dir_models $dir_output $dncon4_file $dist_file");
...
if(-e "$ALL_scores/feature_dncon4_short-range.$targetname" and -e "$ALL_scores/feature_dncon4_medium-range.$targetname" and -e "$ALL_scores/feature_dncon4_long-range.$targetname")
{
	...
	print("perl $H_script/P2_combine_score_into_SVMformat_nosubfix_fea34.pl $dir_models $dir_output/ALL_scores/ $targetname  $dir_output/feat.txt $native_score\n");
	$status = system("perl $H_script/P2_combine_score_into_SVMformat_nosubfix_fea34.pl $dir_models $dir_output/ALL_scores/ $targetname  $dir_output/feat.txt $native_score");
	if($status)# if failed, should we use at least one score?
	{
	...
	print("perl $H_script/P3_make_prediction_by_10folds.pl $dir_output/feat.txt test $H_script/python_lib/DeepRank3_fea34_models/ $dir_output/10_fold_prediction/\n");
	$status = system("perl $H_script/P3_make_prediction_by_10folds.pl $dir_output/feat.txt test $H_script/python_lib/DeepRank3_fea34_models/ $dir_output/10_fold_prediction/");
	if($status)# if failed, should we use at least one score?
	{
	...
	print("perl $H_script/P4_add_prediction2feature_feature34_10folds_avg.pl $dir_output/feat.txt  test $dir_output/10_fold_prediction/  $dir_output/feat_with10folds.txt\n");
	$status = system("perl $H_script/P4_add_prediction2feature_feature34_10folds_avg.pl $dir_output/feat.txt  test $dir_output/10_fold_prediction/  $dir_output/feat_with10folds.txt");
	if($status)# if failed, should we use at least one score?
	{
		die "Failed to run <P1_add_prediction2feature_feature23_10folds.pl>\n";
	}

	print("THEANO_FLAGS=floatX=float32,device=cpu python $H_script/python_lib/predict_main_auto_NN_single.py nadam $dir_output/feat_with10folds.txt $H_script/python_lib/DeepRank3_fea34_models/10fold_2nd_stage_avg_optnadam_htsigmoid_hn5   $dir_output/feat_gdt_predict.txt\n");
	$status = system("THEANO_FLAGS=floatX=float32,device=cpu python $H_script/python_lib/predict_main_auto_NN_single.py nadam $dir_output/feat_with10folds.txt $H_script/python_lib/DeepRank3_fea34_models/10fold_2nd_stage_avg_optnadam_htsigmoid_hn5   $dir_output/feat_gdt_predict.txt");
	if($status)# if failed, should we use at least one score?
	{
		die "Failed to run the prediction for gdt\n";
	}
	...
	print("perl $H_script/P5_add_prediction2feature_feature35.pl $dir_output/feat.txt   $dir_output/feat_gdt_predict.txt  $dir_output/feat_gdt_predict_withall.txt\n");
	$status = system("perl $H_script/P5_add_prediction2feature_feature35.pl $dir_output/feat.txt   $dir_output/feat_gdt_predict.txt  $dir_output/feat_gdt_predict_withall.txt");
	if($status)# if failed, should we use at least one score?
	{
		die "Failed to run the prediction adding for gdt\n";
	}

	$status = system("perl $H_script/P6_get_model2prediction.pl $dir_output/feat.txt  $dir_output/feat_gdt_predict.txt  $dir_output/DeepRank3_Cluster_unsort.txt");
	if($status)# if failed, should we use at least one score?
	{
		die "Failed to get final prediction for gdt\n";
	}
	
	system("perl $H_script/sort_deep_qa_score.pl $dir_output/DeepRank3_Cluster_unsort.txt $dir_output/DeepRank3_Cluster.txt");
...
print "\nFinal prediction: $dir_output/DeepRank3_Cluster.txt\n\n";
