...
###################################################################
  if (@ARGV != 3)
    { # @ARGV used in scalar context = number of args
		print "For casp11\n";
	  print("This script tries to normalize the dfire score, so we can use the normalized dfire score to train NN network.\n");
	  print "perl $PROGRAM_NAME addr_sequence(*.fasta) addr_not_normed_dfire_score(*.DFIRE_score) addr_output\n";
	  
	  print "For example:\n\n";
	  print "perl $PROGRAM_NAME ../../test/casp9_seq/T0516.fasta ../../test/test_dfire_T0516 ../../test/test_dfire_T0516_normed\n";
	  exit(0);
	}
...
{# normalize the score
    ...
    if($count>0)
	{
		print "$input,$out, $count number of models output of range [$min,$max]\n";
	}
