...
`cp -ar  $dir_models $models_DeepQA`;
...
if ($i == $thread_num && $post_process == 0)
{
	...
	{	
	 	...
	 	if($distmap_finish)
			...
			$checkfile11="$dir_output/ALL_scores/feature_dist_ssim.".$targetname;
                        ...
                        if((-e $checkfile1 and -e $checkfile2 and -e $checkfile3 and -e $checkfile4 and -e $checkfile5 and -e $checkfile6 and -e $checkfile7 and -e $checkfile8 and -e $checkfile9 and -e $checkfile10 and -e $checkfile11) or $distmap_manuallystop == 1)
                        {
                                ...
                                {
                                        print "distrank finished!\n";
                                        $finishtime = time();
                                        $model_diff_hrs = ($finishtime - $checktime)/3600;
                                        open(STATUS,">>$dir_output/modelling_status.txt")|| die "Failed to write $dir_output/modelling_status.txt \n";
                                        print STATUS "distrank finished! ($model_diff_hrs hr)\n";
                                        close STATUS;
                                        $distmap_finish = 0;
                                        next;
                                }else{
                                        ...
                                        $model_diff_hrs = ($finishtime - $checktime)/3600;
                                        open(STATUS,">>$dir_output/modelling_status.txt")|| die "Failed to write $dir_output/modelling_status.txt \n";
                                        print STATUS "distrank took >$EXEC_LIMIT_HRS hr! Automatically stopped! Ran ($model_diff_hrs hr)\n";
                                        close STATUS;   
                                        $dncon4_finish = 0;
                                        next;                   
                                }
		...
		print "!!!!!! OPUS:".$OPUS_finish."\tlocal_global:".$local_global_finish."\tRF_SRS:".$RF_SRS_finish."\tRWplus:".$RWplus_finish."\tdope:".$dope_finish."\tmodfoldclust2:".$modfoldclust2_finish."\tpcons:".$pcons_finish."\tpairwise:".$pairwise_finish."\tvoronota:".$voronota_finish."\tDeepQA:".$DeepQA_finish."\tProq3:".$Proq3_finish."\tSBROD:".$SBROD_finish."\tdncon2:".$dncon2_finish."\tdncon4:".$dncon4_finish."\tdistmap:".$distmap_finish." !!!!!! \n\n";
		
		
		if($OPUS_finish== 0 and $local_global_finish== 0 and $RF_SRS_finish== 0 and $RWplus_finish== 0 and $dope_finish== 0 and $modfoldclust2_finish== 0 and $pcons_finish== 0 and $pairwise_finish== 0  and $voronota_finish== 0 and $DeepQA_finish== 0 and $Proq3_finish== 0 and $SBROD_finish== 0 and $dncon2_finish== 0 and $dncon4_finish==0 and $distmap_finish==0)
		{
			last;
		}
