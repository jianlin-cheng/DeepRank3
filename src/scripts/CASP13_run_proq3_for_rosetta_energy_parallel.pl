...
`rm $path_out/Proq3_parallel_*.is_*`;
...
for ($i = 0; $i < @fork_methods; $i++)
{
	...
	elsif ($kidpid == 0)
	{
		...
		### generate proq3
		$res = "$path_out/Proq3_parallel_$i.is_running";
		print LOG "\t$tool  -profile  $fasta_seq     $path_out/parallel_$i/* -outpath $path_out/parallel_out_$i -deep no  -repack no &>> $res\n";
		$return = system("$tool  -profile  $fasta_seq     $path_out/parallel_$i/* -outpath $path_out/parallel_out_$i -deep no  -repack no &>> $res");
		if($return)
		{
		 print "$tool  -profile  $fasta_seq     $path_out/parallel_$i/* -outpath $path_out/parallel_out_$i -deep no  -repack no &>> $res fails !\n";
		 exit(0);
		}
		...
		if(-e "$path_out/Proq3_parallel_$i.is_running")
		{
			system("mv $path_out/Proq3_parallel_$i.is_running $path_out/Proq3_parallel_$i.is_finished");
		}
		open(TMP,">>$path_out/Proq3_parallel_$i.is_finished");
		print TMP "proq parallel $i modeling finished within $method_diff_hrs hrs!\n\n";
		close TMP;		
		exit 0;
...
if ($i == $thread_num && $post_process == 0)
{
	...
	while(1)
	{
		...
		{
			$checkfile = "$path_out/Proq3_parallel_$j.is_finished";
			if(-e $checkfile)
			{
				$method_finish_num ++;
				next;
...
foreach $file (@proqfiles)
{
	...
	$line = shift @content;
	chomp $line;
	@tmp2 = split(/\s/,$line); #proq2 ProQRosCenD ProQRosFAD proq3
	$proq2 = $tmp2[0];
	...
	if(exists($proq2_hash{$modelname}))
	{
	  die "duplicate $modelname in $dir_models\n";
	}else{
	  $proq2_hash{$modelname} = $proq2;
	}

	if(exists($proq3_hash{$modelname}))
	{
	  die "duplicate $modelname in $dir_models\n";
	}else{
	  $proq3_hash{$modelname} = $proq3;
	}

	if(exists($proq3_ProQRosCenD_hash{$modelname}))
	{
	  die "duplicate $modelname in $dir_models\n";
	}else{
	  $proq3_ProQRosCenD_hash{$modelname} = $proq3_ProQRosCenD;
	}

	if(exists($proq3_ProQRosFAD_hash{$modelname}))
	{
	  die "duplicate $modelname in $dir_models\n";
	}else{
	  $proq3_ProQRosFAD_hash{$modelname} = $proq3_ProQRosFAD;
	}
...
=pod
$outfolder_score1 = $path_out.'/feature_proq2_local.txt';
$outfolder_score2 = $path_out.'/feature_proq3_local.txt';
$outfolder_score3 = $path_out.'/feature_proq3_ProQRosCenD_local.txt';
$outfolder_score4 = $path_out.'/feature_proq3_ProQRosFAD_local.txt';
...
foreach $file (@proqfiles)
{
	...
	$proq2_local = "";
	$proq3_ProQRosCenD_local = "";
	$proq3_ProQRosFAD_local = "";
	$proq3_local = "";
	foreach $line (@content)
	{
		...
		@tmp2 = split(/\s/,$line); #proq2 ProQRosCenD ProQRosFAD proq3
		  ...
		  $proq2_score_convert = 3* sqrt((1-$tmp2[0])/$tmp2[0]);
		  $proq3_ProQRosCenD_score_convert = 3*  sqrt((1-$tmp2[1])/$tmp2[1]);
		  $proq3_ProQRosFAD_convert = 3*  sqrt((1-$tmp2[2])/$tmp2[2]);
		  $proq3_score_convert = 3*  sqrt((1-$tmp2[3])/$tmp2[3]);
		  
		  if($proq2_score_convert>15)
		  {
			$proq2_score_convert=15;
		  }
		  if($proq2_score_convert<0)
		  {
			$proq2_score_convert=0;
		  }
		  
		  if($proq3_ProQRosCenD_score_convert>15)
		  {
			$proq3_ProQRosCenD_score_convert=15;
		  }
		  if($proq3_ProQRosCenD_score_convert<0)
		  {
			$proq3_ProQRosCenD_score_convert=0;
		  }
		  
		  if($proq3_ProQRosFAD_convert>15)
		  {
			$proq3_ProQRosFAD_convert=15;
		  }
		  if($proq3_ProQRosFAD_convert<0)
		  {
			$proq3_ProQRosFAD_convert=0;
		  }
		  
		  if($proq3_score_convert>15)
		  {
			$proq3_score_convert=15;
		  }
		  if($proq3_score_convert<0)
		  {
			$proq3_score_convert=0;
		  }
	  
		$proq2_local .= " ".$proq2_score_convert;
		$proq3_ProQRosCenD_local .= " ".$proq3_ProQRosCenD_score_convert;
		$proq3_ProQRosFAD_local.= " ".$proq3_ProQRosFAD_convert;
		$proq3_local .= " ".$proq3_score_convert;
	}

	 
	if(exists($proq2_hash{$modelname}))
	{
	  $proq2_hash{$modelname} .= $proq2_local;
	...
	if(exists($proq3_hash{$modelname}))
	{
	  $proq3_hash{$modelname} .= $proq3_local;
	}else{
	  die "no global score for $modelname in $dir_models\n";
	}
	...
	if(exists($proq3_ProQRosCenD_hash{$modelname}))
	{
	  $proq3_ProQRosCenD_hash{$modelname} .= $proq3_ProQRosCenD_local;
	}else{
	  die "no global score for $modelname in $dir_models\n";
	}
	...
	if(exists($proq3_ProQRosFAD_hash{$modelname}))
	{
	  $proq3_ProQRosFAD_hash{$modelname} .= $proq3_ProQRosFAD_local;
	}else{
	  die "no global score for $modelname in $dir_models\n";
	}
  
	print PROQ2 "$proq2_local\n";
	print PROQ3 "$proq3_local\n";
	print PROQ3LOW "$proq3_ProQRosCenD_local\n";
	print PROQ3HIGH "$proq3_ProQRosFAD_local\n";
...
$outfolder_score1 = $path_out.'/feature_proq2_local_global.txt';
$outfolder_score2 = $path_out.'/feature_proq3_local_global.txt';
$outfolder_score3 = $path_out.'/feature_proq3_ProQRosCenD_local_global.txt';
$outfolder_score4 = $path_out.'/feature_proq3_ProQRosFAD_local_global.txt';
...
foreach $mod (keys %proq2_hash)
{
	if(exists($modeprefix2model{$mod}))
	{
		$mod=$modeprefix2model{$mod};
		print PROQ2 "$mod ".$proq2_hash{$mod}."\n";
	}else{
		print "Failed to find $mod in modeprefix2model\n\n";
	}
}

foreach $mod (keys %proq3_hash)
{
	if(exists($modeprefix2model{$mod}))
	{
		$mod=$modeprefix2model{$mod};
		print PROQ3 "$mod ".$proq3_hash{$mod}."\n";
	}else{
		print "Failed to find $mod in modeprefix2model\n\n";
	}
...
foreach $mod (keys %proq3_ProQRosCenD_hash)
{
	if(exists($modeprefix2model{$mod}))
	{
		$mod=$modeprefix2model{$mod};
		print PROQ3LOW "$mod ".$proq3_ProQRosCenD_hash{$mod}."\n";
	}else{
		print "Failed to find $mod in modeprefix2model\n\n";
	}
...
foreach $mod (keys %proq3_ProQRosFAD_hash)
{
	if(exists($modeprefix2model{$mod}))
	{
		$mod=$modeprefix2model{$mod};
		print PROQ3HIGH "$mod ".$proq3_ProQRosFAD_hash{$mod}."\n";
	}else{
		print "Failed to find $mod in modeprefix2model\n\n";
	}
...
print "1. proq parallel $i modeling finished within $method_diff_hrs hr!\n\n";
